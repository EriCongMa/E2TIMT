...
from validate import validation_sequential_modal_adapter_task
...
def sequential_modal_adapter_train(opt):
    print('Load task sem_ma_train successfully.')
    ...
    # Load triple tuple data
    print('Loading triple tuple data ...')
    ...
    if opt.ocr_tr_encoder_tuning == 'True':
        ...
        model_name_list.append('encoder')
    else:
        freeze_model_list.append(encoder)
        freeze_model_name_list.append('encoder')
    ...
    while(True):
        ...
        visual_adapter_feature_1 = adapter(visual_contextual_feature_1, input = image_1, text = src_text[:, :-1], tgt_mask = tgt_mask)
        visual_adapter_feature_2 = adapter(visual_contextual_feature_2, input = image_1, text = src_text[:, :-1], tgt_mask = tgt_mask)
        visual_adapter_feature_3 = adapter(visual_contextual_feature_3, input = image_1, text = src_text[:, :-1], tgt_mask = tgt_mask)

        textual_contextual_feature = mt_tre_encoder(textual_feature, input = image, text = src_text[:, :-1], tgt_mask = tgt_mask)

        # print('run timt_preds ...')
        timt_preds_1 = tgt_decoder(contextual_feature = visual_adapter_feature_1, input = image_1, text = tgt_text[:, :-1], tgt_mask = tgt_mask)
        timt_preds_2 = tgt_decoder(contextual_feature = visual_adapter_feature_2, input = image_2, text = tgt_text[:, :-1], tgt_mask = tgt_mask)
        timt_preds_3 = tgt_decoder(contextual_feature = visual_adapter_feature_3, input = image_3, text = tgt_text[:, :-1], tgt_mask = tgt_mask)
        ...
        print('End-to-End timt_cost_1 is: {}'.format(timt_cost_1))
        print('End-to-End timt_cost_2 is: {}'.format(timt_cost_2))
        print('End-to-End timt_cost_3 is: {}'.format(timt_cost_3))

        timt_cost = (timt_cost_1 + timt_cost_2 + timt_cost_3) / 3.0

        # print('cl cost calculation part check ...')
        ...
        timt_weight = opt.TIMT_Weight
        cl_weight = opt.CL_Weight

        weighted_timt_cost = timt_weight * timt_cost
        weighted_cl_cost = cl_weight * cl_cost
        
        cost = weighted_timt_cost + weighted_cl_cost
        
        print('timt cost: {:.3f}, cl cost: {}'.format(timt_cost, cl_cost))
        print('Three hyperparameter for cost addition: {:.3f}, {:.3f}'.format( timt_weight, cl_weight ))
        print('Cost After weighted, timt cost: {:.3f}, cl cost: {:.3f}'.format( weighted_timt_cost,  weighted_cl_cost ))
        ...
        print_str=f'step = {iteration+1}, loss = {loss_avg.val():0.5f}, timt_loss = {timt_loss_avg.val():0.5f}, cl_loss = {cl_loss_avg.val():0.5f}, duration = {duration_time:0.2f}s'
        ...
        if (iteration + 1) % opt.valInterval == 0 or iteration == 0: # To see training progress, we also conduct validation when 'iteration == 0' 
            ...
            model_list = [visual_encoder, textual_encoder, encoder, mt_tre_encoder, adapter, tgt_decoder]

            with torch.no_grad():
                valid_loss, current_accuracy, current_bleu, timt_preds_str, mt_preds_str, src_labels, tgt_labels, infer_time, length_of_data = validation_sequential_modal_adapter_task(
            ...
            dashed_line = '-' * 80
            print('Part of TIMT predicted and ground-truth results :')
            head = f'{"Ground Truth":25s} | {"Prediction":25s} | Match-up T/F'
            predicted_result_log = f'{dashed_line}\n{head}\n{dashed_line}\n'
            for gt, pred in zip(tgt_labels[:5], timt_preds_str[:5]):
                gt = gt[:gt.find('[s]')]
